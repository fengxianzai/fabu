<template>
  <div class="container">
    <!-- 面包屑导航栏 -->
    <el-breadcrumb
      separator-class="el-icon-arrow-right"
      style="margin:20px 0 26px 0;border-bottom: 1px solid #dbdcde;padding-bottom: 8px;"
    >
      <el-breadcrumb-item :to="{ path: '/admin/home' }"
        >主页</el-breadcrumb-item
      >
      <el-breadcrumb-item>运维管理</el-breadcrumb-item>
      <el-breadcrumb-item>运维台账</el-breadcrumb-item>
    </el-breadcrumb>

    <!-- 搜索和添加事件 -->
    <el-row type="flex" justify="start">
      <el-col :span="6" style="margin-right:10px">
        <el-input placeholder="输入你要搜索的内容" clearable>
          <el-button slot="append" icon="el-icon-search"></el-button>
        </el-input>
      </el-col>
      <el-col :span="2" style="margin-right:20px">
        <el-button type="primary" size="middle" @click="addDialogVisible = true"
          >添加事件</el-button
        >
      </el-col>
      <el-col :span="2">
        <el-button type="primary" size="middle">导入事件</el-button>
      </el-col>
    </el-row>

    <!-- 运维台账table -->
    <el-row>
      <el-col>
        <el-table
          :data="eventData"
          border
          highlight-current-row
          :cell-style="rowClass"
          :header-cell-style="headClass"
          :header-cell-class-name="cellClass"
          style="width: 100%;margin-top:10px;background:#fafafb"
        >
          <el-table-column fixed label="操作" width="100">
            <template slot-scope="scope">
              <el-button @click="openEdit(scope.row)" type="text" size="small"
                >编辑</el-button
              >
              <el-button
                type="text"
                size="small"
                @click="removeEvent(scope.row)"
                >删除</el-button
              >
            </template>
          </el-table-column>
          <el-table-column prop="id" width="80" label="序号"> </el-table-column>
          <el-table-column
            prop="system_name"
            label="系统名称"
            width="140"
            column-key="date"
            :filters="[
              { text: '法规文件管理系统', value: '法规文件管理系统' },
              { text: 'IR运维系统', value: 'IR运维系统' },
            ]"
            :filter-method="filterHandler"
          >
          </el-table-column>
          <el-table-column
            prop="request_time"
            label="接单时间"
            sortable
            width="160"
          ></el-table-column>
          <el-table-column
            prop="finish_time"
            label="完成时间"
            sortable
            width="160"
          >
          </el-table-column>
          <el-table-column
            prop="request_name"
            label="发起人"
            column-key="date"
            :filters="[
              { text: '黄凯', value: '黄凯' },
              { text: '高东晓', value: '高东晓' },
              { text: '李阳', value: '李阳' },
              { text: '邵宇', value: '邵宇' },
              { text: '彭江涛', value: '彭江涛' },
              { text: '周善非', value: '周善非' },
              { text: '甘博文', value: '甘博文' },
            ]"
            :filter-method="filterHandler"
          >
          </el-table-column>
          <el-table-column prop="date_range" label="班次"> </el-table-column>
          <el-table-column prop="trouble_type" label="故障类型">
          </el-table-column>
          <el-table-column prop="trouble_title" label="故障标题" width="180">
          </el-table-column>
          <el-table-column prop="trouble_content" label="故障现象" width="380">
          </el-table-column>
          <el-table-column prop="solve_name" label="对应人"> </el-table-column>
          <el-table-column prop="cause_analysis" label="原因分析" width="380">
          </el-table-column>
          <el-table-column prop="solve_method" label="解决方法" width="220">
          </el-table-column>
          <el-table-column prop="confirm_name" label="确认人">
          </el-table-column>
          <el-table-column prop="event_state" label="事件状态">
          </el-table-column>
          <el-table-column prop="use_hours" label="对应工时/h" width="100">
          </el-table-column>
        </el-table>
      </el-col>

      <!-- 分页区域 -->
      <el-col class="pagediv" :offset="10">
        <el-pagination
          background
          @size-change="handleSizeChange"
          @current-change="handleCurrentChange"
          :current-page="queryInfo.pagenum"
          :page-sizes="[3, 5, 10]"
          :page-size="queryInfo.pagesize"
          layout="total, sizes, prev, pager, next, jumper"
          :total="total"
        >
        </el-pagination>
      </el-col>
    </el-row>
    <!-- 添加事件的对话框 -->
    <el-row>
      <el-col>
        <el-dialog
          title="添加事件"
          :visible.sync="addDialogVisible"
          width="50%"
          @close="addDialogClosed"
        >
          <!-- 主体区域 -->
          <el-form
            :model="addEventForm"
            ref="addUserFormRef"
            label-width="70px"
          >
            <el-row>
              <el-col :span="10">
                <el-form-item
                  label="系统名称: "
                  prop="system_name"
                  label-width="100px"
                >
                  <el-input v-model="addEventForm.system_name"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12" :offset="2">
                <div class="block">
                  <span class="demonstration">接单时间: </span>
                  <el-date-picker
                    v-model="addEventForm.request_time"
                    type="datetime"
                    placeholder="选择日期时间"
                  >
                  </el-date-picker>
                </div>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="9" style="margin-left:27px">
                <el-form-item label="发起人:" prop="rid">
                  <el-select
                    v-model="addEventForm.request_name"
                    placeholder="选择发起人"
                  >
                    <el-option
                      v-for="item in request_names"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    >
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12" style="margin-left:52px">
                <div class="block">
                  <span class="demonstration">完成时间: </span>
                  <el-date-picker
                    v-model="addEventForm.finish_time"
                    type="datetime"
                    placeholder="选择日期时间"
                  >
                  </el-date-picker>
                </div>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="10">
                <el-form-item
                  label="故障类型: "
                  prop="trouble_type"
                  label-width="100px"
                >
                  <el-input v-model="addEventForm.trouble_type"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12" style="margin-left:18px">
                <el-form-item
                  label="故障标题: "
                  prop="trouble_title"
                  label-width="100px"
                >
                  <el-input v-model="addEventForm.trouble_title"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col>
                <el-form-item
                  label="故障现象: "
                  prop="trouble_content"
                  label-width="100px"
                >
                  <el-input
                    type="textarea"
                    autosize
                    placeholder="请输入内容"
                    v-model="addEventForm.trouble_content"
                  >
                  </el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col>
                <el-form-item
                  label="原因分析: "
                  prop="cause_analysis"
                  label-width="100px"
                >
                  <el-input
                    type="textarea"
                    autosize
                    placeholder="请输入内容"
                    v-model="addEventForm.cause_analysis"
                  >
                  </el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col>
                <el-form-item
                  label="解决方法: "
                  prop="solve_method"
                  label-width="100px"
                >
                  <el-input
                    type="textarea"
                    autosize
                    placeholder="请输入内容"
                    v-model="addEventForm.solve_method"
                  >
                  </el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="12" style="margin-left:27px">
                <el-form-item label="对应人:" prop="solve_name">
                  <el-select
                    v-model="addEventForm.solve_name"
                    placeholder="选择对应人"
                  >
                    <el-option
                      v-for="item in solve_name"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    >
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12" style="margin-left:27px">
                <el-form-item label="确认人:" prop="rid">
                  <el-select
                    v-model="addEventForm.confirm_name"
                    placeholder="选择确认人"
                  >
                    <el-option
                      v-for="item in request_names"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    >
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="12" style="margin-left:27px">
                <el-form-item
                  label="事件状态:"
                  prop="event_state"
                  label-width="50"
                >
                  <el-select
                    v-model="addEventForm.event_state"
                    placeholder="选择状态"
                  >
                    <el-option
                      v-for="item in event_state"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    >
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12" style="margin-left:54px">
                <el-form-item label="班次:" prop="date_range" label-width="50">
                  <el-select
                    v-model="addEventForm.date_range"
                    placeholder="选择状态"
                  >
                    <el-option
                      v-for="item in date_range"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    >
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item
                  label="对应工时: "
                  prop="use_hours"
                  label-width="100px"
                >
                  <el-input v-model="addEventForm.use_hours"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
          <!-- 底部区域 -->
          <span slot="footer" class="dialog-footer">
            <el-button @click="addDialogVisible = false">取 消</el-button>
            <el-button type="primary" @click="addEvent">确 定</el-button>
          </span>
        </el-dialog>
      </el-col>
    </el-row>
    <!-- 编辑事件的对话框 -->
    <el-row>
      <el-col>
        <el-dialog
          title="编辑事件"
          :visible.sync="editDialogVisible"
          width="50%"
          @close="editDialogVisible"
        >
          <!-- 主体区域 -->
          <el-form
            :model="editEventForm"
            ref="editUserFormRef"
            label-width="70px"
          >
            <el-row>
              <el-col :span="10">
                <el-form-item
                  label="系统名称: "
                  prop="system_name"
                  label-width="100px"
                >
                  <el-input v-model="editEventForm.system_name"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12" :offset="2">
                <div class="block">
                  <span class="demonstration">接单时间: </span>
                  <el-date-picker
                    v-model="editEventForm.request_time"
                    type="datetime"
                    placeholder="选择日期时间"
                  >
                  </el-date-picker>
                </div>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="9" style="margin-left:27px">
                <el-form-item label="发起人:" prop="rid">
                  <el-select
                    v-model="editEventForm.request_name"
                    placeholder="选择发起人"
                  >
                    <el-option
                      v-for="item in request_names"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    >
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12" style="margin-left:52px">
                <div class="block">
                  <span class="demonstration">完成时间: </span>
                  <el-date-picker
                    v-model="editEventForm.finish_time"
                    type="datetime"
                    placeholder="选择日期时间"
                  >
                  </el-date-picker>
                </div>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="10">
                <el-form-item
                  label="故障类型: "
                  prop="trouble_type"
                  label-width="100px"
                >
                  <el-input v-model="editEventForm.trouble_type"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12" style="margin-left:18px">
                <el-form-item
                  label="故障标题: "
                  prop="trouble_title"
                  label-width="100px"
                >
                  <el-input v-model="editEventForm.trouble_title"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col>
                <el-form-item
                  label="故障现象: "
                  prop="trouble_content"
                  label-width="100px"
                >
                  <el-input
                    type="textarea"
                    autosize
                    placeholder="请输入内容"
                    v-model="editEventForm.trouble_content"
                  >
                  </el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col>
                <el-form-item
                  label="原因分析: "
                  prop="cause_analysis"
                  label-width="100px"
                >
                  <el-input
                    type="textarea"
                    autosize
                    placeholder="请输入内容"
                    v-model="editEventForm.cause_analysis"
                  >
                  </el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col>
                <el-form-item
                  label="解决方法: "
                  prop="solve_method"
                  label-width="100px"
                >
                  <el-input
                    type="textarea"
                    autosize
                    placeholder="请输入内容"
                    v-model="editEventForm.solve_method"
                  >
                  </el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="12" style="margin-left:27px">
                <el-form-item label="对应人:" prop="solve_name">
                  <el-select
                    v-model="editEventForm.solve_name"
                    placeholder="选择对应人"
                  >
                    <el-option
                      v-for="item in solve_name"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    >
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12" style="margin-left:27px">
                <el-form-item label="确认人:" prop="rid">
                  <el-select
                    v-model="editEventForm.confirm_name"
                    placeholder="选择确认人"
                  >
                    <el-option
                      v-for="item in request_names"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    >
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="12" style="margin-left:27px">
                <el-form-item
                  label="事件状态:"
                  prop="event_state"
                  label-width="50"
                >
                  <el-select
                    v-model="editEventForm.event_state"
                    placeholder="选择状态"
                  >
                    <el-option
                      v-for="item in event_state"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    >
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12" style="margin-left:54px">
                <el-form-item label="班次:" prop="date_range" label-width="50">
                  <el-select
                    v-model="editEventForm.date_range"
                    placeholder="选择状态"
                  >
                    <el-option
                      v-for="item in date_range"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    >
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item
                  label="对应工时: "
                  prop="use_hours"
                  label-width="100px"
                >
                  <el-input v-model="editEventForm.use_hours"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
          <!-- 底部区域 -->
          <span slot="footer" class="dialog-footer">
            <el-button @click="editDialogVisible = false">取 消</el-button>
            <el-button type="primary" @click="editEvent()">确 定</el-button>
          </span>
        </el-dialog>
      </el-col>
    </el-row>
  </div>
</template>

<script>
export default {
  data() {
    // 事件单数据
    const eventData = [];
    return {
      eventData,
      //获取事件单列表的参数对象
      queryInfo: {
        query: '',
        pagenum: 1, //当前的页数
        pagesize: 5, //每页的数量
      },
      total: 0,
      addDialogVisible: false,
      editDialogVisible: false,
      //添加事件的表单数据
      addEventForm: {
        id: '',
        system_name: '',
        date_range: '',
        request_time: '',
        finish_time: '',
        request_name: '',
        trouble_type: '',
        trouble_title: '',
        trouble_content: '',
        solve_name: '',
        cause_analysis: '',
        solve_method: '',
        confirm_name: '',
        event_state: '',
        use_hours: '',
      },
      //编辑事件的表单数据
      editEventForm: {
        id: '',
        system_name: '',
        date_range: '',
        request_time: '',
        finish_time: '',
        request_name: '',
        trouble_type: '',
        trouble_title: '',
        trouble_content: '',
        solve_name: '',
        cause_analysis: '',
        solve_method: '',
        confirm_name: '',
        event_state: '',
        use_hours: '',
      },
      // 用户发起人
      request_names: [
        {
          value: '黄凯',
          label: '黄凯',
        },
        {
          value: '高东晓',
          label: '高东晓',
        },
        {
          value: '李阳',
          label: '李阳',
        },
        {
          value: '邵宇',
          label: '邵宇',
        },
        {
          value: '彭江涛',
          label: '彭江涛',
        },
        {
          value: '周善非',
          label: '周善非',
        },
        {
          value: '甘博文',
          label: '甘博文',
        },
      ],
      // 对应人
      solve_name: [
        {
          value: '陈亮',
          label: '陈亮',
        },
        {
          value: '陈亮/王境成',
          label: '陈亮/王境成',
        },
      ],
      // 事件状态
      event_state: [
        {
          value: 'close',
          label: 'close',
        },
        {
          value: 'open',
          label: 'open',
        },
        {
          value: '进行中',
          label: '进行中',
        },
      ],
      // 班次
      date_range: [
        {
          value: '白班',
          label: '白班',
        },
        {
          value: '夜班',
          label: '夜班',
        },
      ],
    };
  },
  // 请求事件数据
  created() {
    this.getEventsList();
  },
  methods: {
    // 禁用表格头部多选按钮
    cellClass(row) {
      if (row.columnIndex === 0) {
        return 'disabledCheck';
      }
    },
    // 表头样式设置
    headClass() {
      return 'text-align: center;background:#4d4c50;color:#fff';
    },
    // 表格样式设置
    rowClass() {
      return 'text-align: center;background:#fafafb;';
    },
    // 每个事件单的信息
    handleClick(rawDate) {
      console.log(rawDate);
    },
    //监听 pagesize 改变的事件
    handleSizeChange(newSize) {
      this.queryInfo.pagesize = newSize; //重新指定每页数据量
      this.getEventsList(); //带着新的分页请求获取数据
    },
    //监听 页码值 改变的事件
    handleCurrentChange(newPage) {
      this.queryInfo.pagenum = newPage; //重新指定当前页
      this.getEventsList(); //带着新的分页请求获取数据
    },
    // 删选系统名
    filterHandler(value, row, column) {
      const property = column['property'];
      return row[property] === value;
    },
    //监听添加事件对话框的关闭状态
    addDialogClosed() {
      this.$refs.addUserFormRef.resetFields();
    },
    //监听编辑事件对话框的关闭状态
    editDialogClosed() {
      this.$refs.editDialogClosed.resetFields();
    },
    // 请求获取事件列表
    async getEventsList() {
      await this.$axios
        .get('/ywrd/events', {
          params: this.queryInfo,
        })
        .then((res) => {
          this.eventData = res.data.events;
          this.total = res.data.totalpage;
        })
        .catch((error) => {
          //回调函数
          //请求失败或者then里面的代码出现bug的时候
          console.log('http请求失败');
          console.log(error); //期望从服务端返回错误信息
        });
    },

    // 添加事件
    async addEvent() {
      await this.$axios
        .post('/ywrd/addEvent', this.addEventForm)
        .then((res) => {
          //请求成功以后的回调函数
          if (res.data.success) {
            this.getEventsList();
            this.addDialogVisible = false;
            this.$message({
              type: 'success',
              message: '添加成功！',
            });
          } else {
            this.$message({
              type: 'success',
              message: '添加失败！',
            });
          }
        });
    },
    // 打开编辑框
    openEdit(rawDate) {
      this.editDialogVisible = true;
      this.editEventForm.id = rawDate.id;
      this.editEventForm.system_name = rawDate.system_name;
      this.editEventForm.date_range = rawDate.date_range;
      this.editEventForm.request_time = rawDate.request_time;
      this.editEventForm.finish_time = rawDate.finish_time;
      this.editEventForm.request_name = rawDate.request_name;
      this.editEventForm.trouble_type = rawDate.trouble_type;
      this.editEventForm.trouble_title = rawDate.trouble_title;
      this.editEventForm.trouble_content = rawDate.trouble_content;
      this.editEventForm.solve_name = rawDate.solve_name;
      this.editEventForm.cause_analysis = rawDate.cause_analysis;
      this.editEventForm.solve_method = rawDate.solve_method;
      this.editEventForm.confirm_name = rawDate.confirm_name;
      this.editEventForm.event_state = rawDate.event_state;
      this.editEventForm.use_hours = rawDate.use_hours;
    },
    // 编辑事件
    async editEvent() {
      await this.$axios
        .put('/ywrd/updateEvent', this.editEventForm)
        .then((res) => {
          //请求成功以后的回调函数
          if (res.data.success) {
            this.getEventsList();
            this.editDialogVisible = false;
            this.$message({
              type: 'success',
              message: '事件更新成功！',
            });
          } else {
            this.$message({
              type: 'success',
              message: '事件更新失败！',
            });
          }
        });
    },
    // 根据用户ID删除用户信息
    async removeEvent(raw) {
      const eventID = raw.id; //获取事件ID
      await this.$axios.delete('/ywrd/removeEvent/' + eventID).then((res) => {
        if (res.data.success) {
          this.getEventsList();
          this.$message({
            type: 'success',
            message: res.data.info,
          });
        } else {
          this.$message({
            type: 'success',
            message: res.data.info,
          });
        }
      });
    },
  },
};
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
/* 分页 start */
.pagediv {
  margin: 40px 0px 40px 0px;
  text-align: center;
}
el-pagination {
  width: 200px;
}
/* 分页 end */

.demonstration {
  margin-right: 4px;
}
</style>
